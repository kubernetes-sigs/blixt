...
use crate::infrastructure::{
    AsyncCommand, AsyncCommandError, ContainerImageError, ContainerRuntime, ContainerState,
    Workload, WorkloadImageTag, container_host_env,
};
...
impl KindCluster {
    ...
    pub fn new<T: AsRef<str>>(name: T, runtime: ContainerRuntime) -> Result<Self> {
        Ok(KindCluster {
    ...
    /// set the container host connection
    /// if unset defaults to the environment variable `$CONTAINER_HOST` or
    /// falls back to the system podman socket at `unix:///run/podman/podman.sock`
    pub fn set_container_host<T: AsRef<str>>(&mut self, container_host: T) {
        self.container_host = Some(container_host.as_ref().to_string())
    }
    ...
    /// get the container id corresponding to the cluster
    pub async fn container_id(&self) -> Result<String> {
        let filter = format!(r#"name=^{}-{}$"#, self.name, self.role);
        let (output, _stderr) = AsyncCommand::new(
            self.runtime.to_string(),
            &["ps", "--no-trunc", "-q", "--filter", filter.as_str()],
        )
        .env(container_host_env(self.container_host.as_ref()))
        .read()
        .await
        .map_err(|e| {
            KindClusterError::Execution(format!("Failed to get container id for {}", self.name), e)
        })?;

        let output = output.trim_end().to_string();
        match output.trim_end().len() == 64 {
            true => Ok(output),
            false => Err(KindClusterError::NotFound(self.name.to_string()).into()),
        }
    }
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        match self.state().await? {
            ...
            ContainerState::Exited => {
                ...
                AsyncCommand::new(self.runtime.to_string(), &["start", container_id.as_str()])
                    .env(container_host_env(self.container_host.as_ref()))
                    .run()
                    .await
                    .map_err(|e| {
                        KindClusterError::Execution(
                            format!("Failed to start container {container_id}"),
                            e,
                        )
                    })
            }
            ContainerState::NotFound => {
                AsyncCommand::new("kind", &["create", "cluster", "--name", name])
                    .run()
                    .await
                    .map_err(|e| {
                        KindClusterError::Execution(
                            format!("Failed to create kind cluster {name}"),
                            e,
                        )
                    })
            }
    ...
    pub async fn delete(&self) -> Result<()> {
        ...
        AsyncCommand::new(self.runtime.to_string(), &["stop", container_id.as_str()])
            .env(container_host_env(self.container_host.as_ref()))
            .run()
            .await
            .map_err(|e| {
                KindClusterError::Execution(format!("Failed to stop container {container_id}"), e)
            })?;
        AsyncCommand::new(self.runtime.to_string(), &["rm", container_id.as_str()])
            .env(container_host_env(self.container_host.as_ref()))
            .run()
            .await
            .map_err(|e| {
                KindClusterError::Execution(format!("Failed to remove container {container_id}"), e)
                    .into()
            })
    }

    /// get the clusters state
    pub async fn state(&self) -> Result<ContainerState> {
        ...
        .map_err(|e| {
            KindClusterError::Execution(
                format!("Failed to get container state for {}", self.name),
                e,
            )
        })?;
    ...
    /// errors with `KindError::ContainerState` in case the cluster is not ready
    pub async fn ready(&self) -> Result<()> {
        let state = self.state().await?;
        match &state {
            ContainerState::Running => Ok(()),
            ContainerState::Exited | ContainerState::NotFound => {
                Err(KindClusterError::ContainerState(self.name.to_string(), state.clone()).into())
            }
        }
    }

    /// mount the ebpf file system on the cluster host
    /// use bind mounts in the containers to access the file system
    pub async fn host_mount_bpf_fs(&self) -> Result<()> {
        ...
        .map_err(|e| {
            KindClusterError::Execution(
                format!("Failed to mount bpf fs on host container {container_id}"),
                e,
            )
            .into()
        })
