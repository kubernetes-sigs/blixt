...
use crate::{Error, Result, gatewayclass_utils};
...
impl GatewayController {
    ...
    pub async fn reconcile(gateway: Arc<Gateway>, ctx: Arc<GatewayController>) -> Result<Action> {
        ...
        let gateway_api: Api<Gateway> = Api::namespaced(ctx.k8s_client.clone(), &ns);
        ...
        let service_api: Api<Service> = Api::namespaced(ctx.k8s_client.clone(), &ns);
        ...
        let svc_status: &ServiceStatus = match service.status.as_ref().ok_or(
            Error::LoadBalancerError("Loadbalancer service status not found".to_string()),
        ) {
            Ok(status) => status,
            Err(error) => {
                invalid_lb_condition.message = error.to_string();
