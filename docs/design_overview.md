...
erDiagram
    ...
    Dataplane one to one Kernel : "controls via SysCalls"
    ...
    Controlplane one to one or many k8sApi : "interacts with"
```


# GatewayApi Data Flow

```mermaid
flowchart LR
    User[/User/]
    k8sResources(["k8s Resources\GatewayApi CRDs"])
