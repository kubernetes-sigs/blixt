...
pub mod backends_server {
    ...
    #[async_trait]
    pub trait Backends: std::marker::Send + std::marker::Sync + 'static {
    ...
    impl<T> Clone for BackendsServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "backends.backends";
    impl<T> tonic::server::NamedService for BackendsServer<T> {
