...
use crate::*;
use k8s_gateway_api::{
    Gateway, GatewayAddress, GatewaySpec, GatewayStatus, Listener, ListenerStatus, RouteGroupKind,
};
use kube::{
...
pub fn set_gateway_status_addresses(gateway: &mut Gateway, svc: &Service) {
    let mut gw_addrs: Vec<GatewayAddress> = vec![];

    if let Some(load_balancer) = &svc.status.as_ref().unwrap().load_balancer {
        if let Some(ingress) = &load_balancer.ingress {
            for addr in ingress {
                if let Some(ip) = &addr.ip {
                    gw_addrs.push(GatewayAddress {
    ...
    gateway.status.as_mut().unwrap().addresses = Some(gw_addrs);
}
...
pub async fn create_endpoint_if_not_exists(ctx: Arc<Context>, svc: &Service) -> Result<()> {
    let mut lb_addr = String::from("");
    if let Some(load_balancer) = &svc.status.as_ref().unwrap().load_balancer {
        if let Some(ingress) = &load_balancer.ingress {
            for addr in ingress {
                if let Some(ip) = &addr.ip {
    ...
    let endpoints_api: Api<Endpoints> =
        Api::namespaced(ctx.client.clone(), svc.namespace().unwrap().as_str());
    let err = endpoints_api.get(svc.name_unchecked().as_str()).await.err();

    if let Some(error) = err {
        if check_if_not_found_err(error) {
            let mut ep_ports: Vec<EndpointPort> = vec![];
            if let Some(ports) = &svc.spec.as_ref().unwrap().ports {
                for port in ports {
                    let mut ep_port = EndpointPort::default();
                    ep_port.port = port.port;
                    ep_port.protocol = port.protocol.clone();
                    ep_ports.push(ep_port);
                }
            }

            let mut obj_meta = ObjectMeta::default();
            obj_meta.name = svc.meta().name.clone();
            obj_meta.namespace = svc.namespace();
...
pub fn get_ingress_ip_len(svc: &Service) -> usize {
    if let Some(lb) = &svc.status.as_ref().unwrap().load_balancer {
...
pub async fn create_svc_for_gateway(ctx: Arc<Context>, gateway: &Gateway) -> Result<Service> {
    ...
    svc_meta.namespace = gateway.namespace();
    svc_meta.generate_name = Some(format!("service-for-gateway-{}-", gateway.name_any()));

    let mut labels = BTreeMap::new();
    labels.insert(GATEWAY_SERVICE_LAEBL.to_string(), gateway.name_any());
...
pub fn update_service_for_gateway(gateway: &Gateway, svc: &mut Service) -> Result<bool> {
    ...
    for listener in &gateway.spec.listeners {
        ...
        port.port = listener.port as i32;
    ...
    let lb_ip: Option<String> = svc.spec.as_ref().unwrap().load_balancer_ip.clone();
    if let Some((addr, ip)) = address.clone().zip(lb_ip.clone()) {
        if ip != addr.value {
            svc.spec.as_mut().unwrap().load_balancer_ip = Some(addr.value.clone());
            updated = true;
        }
    }
    if address.is_none() && lb_ip.is_some() {
        svc.spec.as_mut().unwrap().load_balancer_ip = None;
        updated = true;
    }
    if let Some(ref mut t) = svc.spec.as_mut().unwrap().type_ {
        if t != "LoadBalancer" {
    ...
    if let Some(ref mut svc_ports) = svc.spec.as_mut().unwrap().ports {
        let mut diff = false;
        ...
        svc.spec.as_mut().unwrap().ports = Some(ports);
    }
    Ok(updated)
}
...
pub async fn patch_status(
    ...
    let listeners = status.listeners.clone().unwrap_or(vec![]);
    let conditions = status.conditions.clone().unwrap_or(vec![]);
    let addresses = status.addresses.clone().unwrap_or(vec![]);
...
// Sets the provided conditon on the Gateway object. The condition on the Gateway is only updated
    ...
    if let Some(ref mut status) = gateway.status {
        if let Some(ref mut conditions) = status.conditions {
            for i in 0..conditions.len() {
                if conditions[i].type_ == new_cond.type_ {
                    if conditions[i].status == new_cond.status {
                        // always update the observed generation
                        conditions[i].observed_generation = new_cond.observed_generation;
                        return;
                    }
                    conditions[i] = new_cond.clone();
                    return;
                }
            }
...
// Inspects the provided Gateway and returns an appropriate a type "Accepted" Condition.
        ...
        if let Some(listeners) = &status.listeners {
            for listener in listeners {
                for conditon in &listener.conditions {
                    if conditon.status == "False".to_string() {
                        accepted.status = String::from("False");
                        accepted.reason = String::from("ListenersNotValid");
        ...
        for addr in addresses {
            if let Some(addr_type) = &addr.r#type {
                if addr_type.as_str() != "IPAddress" {
                    accepted.status = String::from("False");
                    accepted.reason = String::from("UnsupportedAddress");
...
pub fn set_listener_status(gateway: &mut Gateway) {
    let gateway_spec: &GatewaySpec = &gateway.spec;
    let mut statuses: Vec<ListenerStatus> = vec![];
    let mut current_listener_statuses: HashMap<String, ListenerStatus> = HashMap::new();

    if let Some(listeners) = &gateway.status.as_ref().unwrap().listeners {
        for listener in listeners {
    ...
    for listener in &gateway_spec.listeners {
        ...
        let (supported_kinds, conditions) =
            get_listener_status(listener, gateway.metadata.generation.unwrap());
...
fn get_listener_status(
    listener: &Listener,
    generation: i64,
) -> (Vec<RouteGroupKind>, Vec<metav1::Condition>) {
    let now = k8s_openapi::apimachinery::pkg::apis::meta::v1::Time(Utc::now());
    let mut supported_kinds: Vec<RouteGroupKind> = vec![];
    let mut conditions: Vec<metav1::Condition> = vec![
        metav1::Condition {
            type_: String::from("ResolvedRefs"),
            status: String::from("True"),
            reason: String::from("ResolvedRefs"),
        ...
        metav1::Condition {
            type_: String::from("Accepted"),
            status: String::from("True"),
            reason: String::from("Accpeted"),
            observed_generation: Some(generation),
            last_transition_time: now.clone(),
            message: String::from("Listener is valid"),
        },
        metav1::Condition {
            type_: String::from("Programmed"),
            status: String::from("True"),
            reason: String::from("Programmed"),
            observed_generation: Some(generation),
            last_transition_time: now,
            message: String::from("Listener is valid"),
        },
    ...
    match listener.protocol.as_str() {
        ...
        "TCP" | "HTTP" | "HTTPS" => {
            supported_kinds.push(RouteGroupKind {
        ...
        _ => {
            update_listener_condition(
                String::from("False"),
                String::from("UnsupportedProtocol"),
                format!(
...
fn check_route_kinds(kind: Option<&str>, rgks: &Vec<RouteGroupKind>) -> Option<String> {
    if rgks.len() == 0 {
        return None;
    }
    ...
    if let Some(k) = kind {
        ...
        if rgk.kind != "TCPRoute" || rgk.kind != "UDPRoute" {
            return Some(format!(
